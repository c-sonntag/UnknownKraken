...
int main(int argc, char **argv) {
    ...
    if (!(decipher_keystore_manager = ue_pgp_keystore_manager_init("res/pem/server_pgp_pub.pem", "res/pem/server_pgp_priv.pem", "res/pem/server_pgp_pub.pem", NULL))) {
    ...
    ue_logger_info("Deciphering message as server...");
    if (!(output_pmsg = ue_message_build_decrypted_as_server(decipher_keystore_manager, received_cmsg))) {
        ue_stacktrace_push_msg("Failed to decrypt message")
        goto clean_up;
    }
    ue_logger_info("Message deciphered");

    ue_logger_info("Recovering plain message header...");
    if (!(plaintext = ue_plain_message_header_to_string(output_pmsg))) {
        ue_stacktrace_push_msg("Failed to convert output plain message header to string")
        goto clean_up;
    }
    ue_logger_debug("Recovered message is : '%s'", plaintext);

    if (ue_plain_message_header_equals(input_pmsg, output_pmsg)) {
        ue_logger_info("Both plain message headers are equals");
    } else {
        ue_stacktrace_push_msg("Comparaison of both message headers failed")
        ue_logger_error("Both plain message header aren't equals");
        goto clean_up;
    }
