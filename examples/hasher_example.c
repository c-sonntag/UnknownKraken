...
int main(int argc, char **argv) {
    ...
    ei_logger_info("Creating new ue_hasher");
    if (!(h = ue_hasher_create())) {
        ei_stacktrace_push_msg("Failed to create ue_hasher")
        goto clean_up;
    }
    ei_logger_info("Has successfully created a new ue_hasher");

    ei_logger_info("Initializing ue_hasher with SHA-256 digest algorithm");
    if (!(ue_hasher_init(h, "sha256"))) {
        ei_stacktrace_push_msg("Failed to initialize ue_hasher with SHA-256 algorithm")
        goto clean_up;
    }
    ei_logger_info("Has successfully initialized ue_hasher");
