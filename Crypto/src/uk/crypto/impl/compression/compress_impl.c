...
bool uk_crypto_deflate_compress(unsigned char *plaintext, size_t plaintext_len, unsigned char **compressed_text, size_t *compressed_len) {
    ...
    if (plaintext_len > ULONG_MAX) {
        uk_utils_stacktrace_push_msg("plaintext_len=%ld > ULONG_MAX=%ld", plaintext_len, ULONG_MAX);
        return false;
    }
...
bool uk_crypto_inflate_decompress(unsigned char *compressed_text, size_t compressed_len, unsigned char **decompressed_text, size_t decompressed_len) {
    ...
    if (decompressed_len > ULONG_MAX) {
        uk_utils_stacktrace_push_msg("plaintext_len=%ld > ULONG_MAX=%ld", decompressed_len, ULONG_MAX);
        return false;
    }
...
bool uk_crypto_deflate_compress_file(FILE *source, FILE *dest, int level) {
    ...
    do {
        ...
        if (r > UINT_MAX) {
            uk_utils_stacktrace_push_msg("Stop compress to prevent data loss because fread result > to UINT_MAX");
        ...
        if (strm.avail_in != 0) {
            uk_utils_stacktrace_push_msg("All input is not use");
            (void)deflateEnd(&strm);
            return false;
        }
