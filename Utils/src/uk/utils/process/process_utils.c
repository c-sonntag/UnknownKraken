 ...
 */
char *uk_utils_get_current_process_name() {
#if defined(_WIN32)
    char *szFileName, *error_buffer;

    uk_utils_safe_alloc(szFileName, char, MAX_PATH + 1);

    if (GetModuleFileNameA(NULL, szFileName, MAX_PATH + 1) == ERROR_INSUFFICIENT_BUFFER) {
        uk_utils_get_last_werror(error_buffer);
        uk_utils_stacktrace_push_msg("GetModuleFileNameA failed error message: %s", error_buffer);
        uk_utils_safe_free(szFileName);
        return NULL;
    }
    ...
    uk_utils_stacktrace_push_msg("Unsupported OS");
    return NULL;
}
