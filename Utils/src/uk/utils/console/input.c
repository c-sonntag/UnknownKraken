...
char *uk_utils_input_string(char *prefix) {
    ...
    printf("%s", prefix);
      ...
      if (fgets(input, 256, stdin)) {
        ...
        for (i = 0; i < 256; i++) {
            if (input[i] != ' ') {
                result = uk_utils_string_create_from(input);
                uk_utils_remove_last_char(result);
                break;
            }
        }
      }
...
static ssize_t get_input_password_unix(char **password, ssize_t max_size, int mask, FILE *stream) {
    ...
    if (!password || !max_size || !stream) {
        uk_utils_stacktrace_push_msg("At least one invalid parameter");
        return -1;
    }
    ...
    if (tcgetattr(0, &old_kbd_mode)) {
        uk_utils_stacktrace_push_msg("tcgetattr failed");
        return -1;
    }
    ...
    if (tcsetattr(0, TCSANOW, &new_kbd_mode)) {
        uk_utils_stacktrace_push_msg("tcsetattr failed with new flags");
        return -1;
    }
    ...
    if (tcsetattr(0, TCSANOW, &old_kbd_mode)) {
        uk_utils_stacktrace_push_msg("tcsetattr failed to set previous flags");
        return -1;
    }

    /* Warn if password truncated */
    if (idx == max_size - 1 && c != '\n') {
        uk_utils_logger_warn("Truncated at %zu chars", max_size - 1);
    }
...
char *uk_utils_input_password(const char *prompt_message, ssize_t max_size) {
    ...
    uk_utils_safe_alloc(password, char, max_size);
    size = 0;
    ...
    if (size < max_size) {
        uk_utils_safe_realloc(password, char, size, max_size - size);
    }
