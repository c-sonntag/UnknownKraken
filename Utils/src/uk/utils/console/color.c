...
char *uk_utils_colorize_string(const char *string, int attribute_id, int foreground_id, int background_id) {
    ...
    colored_length = 8 + string_length + strlen(UnknownKrakenUtils_COLOR_ESCAPE_RESET);

    if (color_is_valid_attribute(attribute_id)) {
        colored_length += 2;
        uk_utils_safe_alloc(attribute, char, 13);
        sprintf(attribute, "%d", attribute_id);
    } else {
        attribute = uk_utils_string_create_from("");
    }

    if (color_is_valid_foreground(foreground_id)) {
        colored_length += 2;
        uk_utils_safe_alloc(foreground, char, 13);
        sprintf(foreground, ";%d", foreground_id);
    } else {
        foreground = uk_utils_string_create_from("");
    }

    if (color_is_valid_background(background_id)) {
        colored_length += 2;
        uk_utils_safe_alloc(background, char, 13);
        sprintf(background, ";%d", background_id);
    } else {
        background = uk_utils_string_create_from("");
    }

    uk_utils_safe_alloc(colored, char, colored_length);

    sprintf(colored, "%c[%s%s%sm%s%s", 0x1B, attribute, foreground, background, string, UnknownKrakenUtils_COLOR_ESCAPE_RESET);
    colored[colored_length - 1] = '\0';

    uk_utils_safe_free(attribute);
    uk_utils_safe_free(foreground);
    uk_utils_safe_free(background);
