...
static bool read_seal_payload(ueum_byte_stream *encoded_message, uecm_crypto_metadata *our_crypto_metadata,
    ...
    if (!ueum_byte_read_next_int(encoded_message, &cipher_data_size)) {
    ...
    ueum_byte_read_next_bytes(encoded_message, &cipher_data, (size_t)cipher_data_size);

    if (!uecm_decipher_cipher_data(cipher_data, (size_t)cipher_data_size,
...
ue_relay_received_message *ue_relay_message_decode(ueum_byte_stream *encoded_message,
    ...
    if (ueum_byte_stream_is_empty(encoded_message)) {
    ...
    ueum_byte_stream_set_position(encoded_message, 0);

    /* Check protocol id */
    ueum_byte_read_next_int(encoded_message, &read_int);
    ...
    received_message->remaining_encoded_route = ueum_byte_stream_create();
    received_message->remaining_encoded_back_route = ueum_byte_stream_create();

    /* Read the encoded route */
    if (!ueum_byte_read_next_stream(encoded_message, received_message->remaining_encoded_route)) {
        ue_relay_received_message_destroy(received_message);
    ...
    if (!ueum_byte_read_next_stream(encoded_message, received_message->remaining_encoded_back_route)) {
        ue_relay_received_message_destroy(received_message);
    ...
    if (received_message->message_id == UNKNOWNECHO_RELAY_MESSAGE_ID_REQUEST ||
        ...
        if (!ueum_byte_stream_is_empty(received_message->remaining_encoded_route)) {
            ei_logger_trace("Remaining route isn't empty, so the message wasn't meant for us. We cannot unseal it, anyway.");
            received_message->payload = ueum_byte_stream_create();
            if (!ueum_byte_read_next_stream(encoded_message, received_message->payload)) {
                ue_relay_received_message_destroy(received_message);
