...
ue_socket_client_channel *client_channel = NULL;
...
void shutdown_client(int sig) {
    ...
    ue_logger_info("Shuting down client...");
   	...
   	client_channel->transmission_state = CLOSING_STATE;
	...
	while (client_channel->running) {
		...
		if (received <= 0 || received == ULLONG_MAX) {
			...
			if (csr_sub_type == CSR_TLS_RESPONSE) {
				...
				if (!(client_channel->tls_csr_context->signed_certificate = client_process_server_response(csr_response,
				...
				if (!(client_channel->cipher_csr_context->signed_certificate = client_process_server_response(csr_response,
				...
				if (!(client_channel->signer_csr_context->signed_certificate = client_process_server_response(csr_response,
...
bool process_message(ue_byte_stream *message) {
	...
	if (!write(client_channel->fds[1], ue_byte_stream_get_data(printer), ue_byte_stream_get_size(printer))) {
		ue_logger_warn("Failed to write the message on console in fd %d", client_channel->fds[1]);
		goto clean_up;
	}
...
bool process_channel_key_response(ue_socket_client_connection *connection, ue_byte_stream *response) {
	...
	if (!(key_owner_certificate = ue_pkcs12_keystore_find_certificate_by_friendly_name(client_channel->signer_keystore,
		...
		if (!process_get_certificate_request(client_channel->signer_keystore, connection, (const unsigned char *)friendly_name, friendly_name_size)) {
...
bool process_channel_key_request(ue_socket_client_connection *connection, ue_byte_stream *request) {
	...
	if (!(client_certificate = ue_pkcs12_keystore_find_certificate_by_friendly_name(client_channel->cipher_keystore, (const unsigned char *)friendly_name, friendly_name_size))) {
		...
		if (!process_get_certificate_request(client_channel->cipher_keystore, connection, (const unsigned char *)friendly_name, friendly_name_size)) {
			ue_stacktrace_push_msg("Failed to get missing certificate");
			goto clean_up;
		}
...
bool send_csr(csr_context *context, int csr_sub_type) {
	...
	if (!send_message(client_channel->connection, client_channel->connection->message_to_send)) {
...
bool ue_socket_client_channel_start() {
    ...
    if (pipe(client_channel->fds) == -1) {
		ue_stacktrace_push_errno();
        return false;
    }

    client_channel->child_pid = fork();
    if (client_channel->child_pid == -1) {
		ue_stacktrace_push_errno();
        return false;
    }

    if (client_channel->child_pid == 0) {
        close(client_channel->fds[1]);
        char f[PATH_MAX + 1];
        sprintf(f, "/dev/fd/%d", client_channel->fds[0]);
...
bool create_keystores(const char *csr_server_host, unsigned short int csr_server_port, char *keystore_password) {
	...
	if (!tls_keystore_exists) {
		...
		if (!send_csr(client_channel->tls_csr_context, CSR_TLS_REQUEST)) {
	...
	if (!cipher_keystore_exists) {
		...
		if (!send_csr(client_channel->cipher_csr_context, CSR_CIPHER_REQUEST)) {
	...
	if (!signer_keystore_exists) {
		...
		if (!send_csr(client_channel->signer_csr_context, CSR_SIGNER_REQUEST)) {
...
bool ue_socket_client_channel_create(char *root_path, char *nickname, const char *csr_server_host, int csr_server_port,
	...
	ue_safe_alloc(client_channel, ue_socket_client_channel, 1);
    client_channel->fd = -1;
	...
	if (!create_keystores(csr_server_host, csr_server_port, client_channel->keystore_password)) {
